describe Carto::Models::User, type: :model do
  ...
  describe '.authenticate' do
    it 'returns the user with the matching email and password' do
      expect(Carto::Models::User.authenticate(user.email, 'traveler')).to eql user
    end

    it 'returns nil if the email does not exist' do
      expect(Carto::Models::User.authenticate('foo@bar.com', 'traveler')).to eql nil
    end

    it 'returns nil if the password does not match' do
      expect(Carto::Models::User.authenticate(user.email, 'foobar')).to eql nil
    end
  end

  describe '.api_authenticate' do
    it 'returns the user with matching api key and secret' do
      expect(Carto::Models::User.api_authenticate(user.api_key, user.api_secret)).to eql user
    end

    it 'returns nil if api key and secret do not match' do
      expect(Carto::Models::User.api_authenticate('foo', 'bar')).to eql nil
    end
  end
